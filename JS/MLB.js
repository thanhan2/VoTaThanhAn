import { MLBData } from "./productData.js";
...
MLBData.slice(0, 9).forEach((it) => {
  loadProductFromDataTab1(it);
});
MLBData.slice(9).forEach((it) => {
...
sortSelect.addEventListener("change", function () {
  ...
  if (sortSelect.value === "Mới nhất") {
    ...
    let newMLBData = MLBData.sort((a, b) => (a.name > b.name ? 1 : -1));
    newMLBData.forEach((item) => {
    ...
    let newMLBData = MLBData.sort((a, b) => (a.price > b.price ? 1 : -1));
    newMLBData.forEach((item) => {
      loadProductFromDataTab1(item);
    });
    handleProductDetail();
